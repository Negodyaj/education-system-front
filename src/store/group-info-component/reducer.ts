...
export function groupInfoComponentReducer(
  state: IGroupInfoComponent = initialState,
  action: GroupInfoComponentActions
): IGroupInfoComponent {
  switch (action.type) {
    ...
    case GROUP_TO_VIEW_WRETCH_LOADED:
      return {
        ...state,
        groupToView: action.payload,
        studentsGroup: action.payload.students,
        isDataLoading: false,
      };
