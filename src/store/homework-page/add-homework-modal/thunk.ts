...
export const getHomework = () => (dispatch: Dispatch) => {
  sendGetRequest<Homework[]>(homeworkUrl, isHomeworkArr).then((homeworks) =>
    dispatch(loadHomeworkForModalSuccess(homeworks))
  );
};
...
export const addHomework = (newHomework: HomeworkInput) => (
  ...
  sendPostRequest<Homework>(`${homeworkUrl}`, isHomework, newHomework).then(
    (homework) => {
      const response = thunkResponseHandler(dispatch, homework);
        ...
        dispatch(
          pushNotification(
            makeNotification(
              'success',
              `Домашняя работа ${
                (response as Homework).description
              } успешно добавленa`
            )
          )
        );
...
export const getCourseForHW = () => (dispatch: Dispatch) => {
  sendGetRequest<Course[]>(homeworkUrl, isCourseArr).then((courses) =>
    dispatch(loadHomeworkForModalSuccess(courses))
  );
};
