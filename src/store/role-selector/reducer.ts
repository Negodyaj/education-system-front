...
export function roleSelectorReducer(
  state: IRoleSelector = initialState,
  action: RoleSelectorActions
): IRoleSelector {
  switch (action.type) {
    ...
    case CURRENT_USER_WAS_LOADED:
      return {
        ...
        isTurnedOn: getCurrentUserFromStorage().roles.length !== 1,
        isDataLoading: false,
      };
    ...
    case TOGGLE_ROLE_SELECTOR:
      return {
        ...state,
        isTurnedOn: state.currentUser ? !state.isTurnedOn : false,
      };
