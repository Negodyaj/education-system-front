...
function DeleteRoleButton(props: { user: User; roleId: number }) {
  ...
  const onDeleteRoleClick = (user: User, roleId: number) => {
    ...
    setConfirmationMessage(
      `Вы точно хотите избавить пользователя ${user.lastName}( ${user.login}) от роли ${Role[roleId]}?`
    );
    setIsModalShow(true);
  };
  const refreshUser = () => {
    ...
    newUsers[index] = {
      ...newUsers[index],
      roles: newUsers[index].roles?.filter((r) => r !== roleID),
    };
  ...
  return (
    <>
      <button
        className="round-button mini-button"
        onClick={() => onDeleteRoleClick(props.user, props.roleId)}>
